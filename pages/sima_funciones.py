...
def validar_subgrupo(df, conn):
    ...
    # Replace invalid subgroups with empty strings
    df['SubgrupoCultivo'] = df['SubgrupoCultivo'].apply(lambda x: x if x in subgrupo else '')

    # Query to get subgroups and corresponding cultivos
    ...
    # Create a dictionary to store the valid subgroups for each cultivo
    subgrupos_cultivos_dict = {}
    for _, row in subgrupos_cultivos.iterrows():
        cultivo = row['cultivo']
    ...
    # Function to validate subgrupo for each row
    def validar_subgrupo_row(row):
        valor_cultivo = row['Cultivo']
    ...
    df['SubgrupoCultivo'] = df.apply(validar_subgrupo_row, axis=1)
