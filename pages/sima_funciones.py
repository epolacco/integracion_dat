...
# FILTRAR CAMPAÑAS (CICLOS)
def filtrar_campana(df,conn):
...
def validar_genetica_semillero(df, conn):
...
# RELLENAR VALORES NA CON O Y CONVERTIR A INT
def fertilizante_int(df):
    columnas = [
...
# LOCALIDADES
def validar_localidad(df,conn):
    
    # Consulta para obtener las localidades válidas
    localidades_query = """SELECT localidad FROM datcrea_tablas.localidades"""
    localidades_df = pd.read_sql(localidades_query, con=conn)
    localidades = localidades_df['localidad'].tolist()

    # Unificar criterio de escritura y validar localidades
    df['Localidad'] = df['Localidad'].astype(str).str.title()
    df['Localidad'] = df['Localidad'].apply(lambda x: x if x in localidades else '')
    return df
...
def validar_departamento(df,conn):
    ...
    df = pd.read_sql(departamentos_query,con=conn)
...
# Herbicidas
def validar_herbicidas(df,conn):
    # Consultar los nombres de herbicidas desde la base de datos
    herbicidas_query = "SELECT nombre FROM datcrea_tablas.agroquimicos WHERE tipo = 'herbicidas'"
    df = pd.read_sql(herbicidas_query, conn)
    herbicidas = df['nombre'].tolist()
    
    # Definir la función de validación
    def validar_herbicida(valor):
        return valor if valor in herbicidas else ''
    
    # Lista de columnas a validar
    columnas_a_validar = [
        'Herbicida 1.1', 'Herbicida 1.2', 'Herbicida 1.3', 'Herbicida 1.4',
        'Herbicida 2.1', 'Herbicida 2.2', 'Herbicida 2.3', 'Herbicida 2.4',
        'Herbicida 3.1', 'Herbicida 3.2', 'Herbicida 3.3', 'Herbicida 3.4',
        'Herbicida 4.1', 'Herbicida 4.2', 'Herbicida 4.3', 'Herbicida 4.4',
        'Herbicida 5.1', 'Herbicida 5.2', 'Herbicida 5.3', 'Herbicida 5.4'
    ]
    
    # Validar cada columna especificada
    for columna in columnas_a_validar:
        if columna in df.columns:
            df[columna] = df[columna].apply(validar_herbicida)
    
    return df

# Fungicida
def validar_fungicida(df, conn):
    # Consultar los nombres de herbicidas desde la base de datos
    fungicidas_query = "SELECT nombre FROM datcrea_tablas.agroquimicos WHERE tipo = 'fungicidas'"
    df = pd.read_sql(fungicidas_query, conn)
    fungicidas = df['nombre'].tolist()
    
    # Definir la función de validación
    def validar_fungicida(valor):
        return valor if valor in fungicidas else ''
    
    # Lista de columnas a validar
    columnas_a_validar = ['FUNG I PRODUCTO', 'FUNG II PRODUCTO']
    
    # Validar cada columna especificada
    for columna in columnas_a_validar:
        if columna in df.columns:
            df[columna] = df[columna].apply(validar_fungicida)
    
    return df


# SUBGRUPOS
def validar_subgrupo(df,conn):
    # Consultar los subgrupos desde la base de datos
    query = "SELECT cultivo, subgrupo FROM cultivo_subgrupo"
    df_subgrupos = pd.read_sql(query, conn)
    ...
    def validar_fila(row):
        ...
        # Filtrar los subgrupos válidos para el cultivo dado
        subgrupos_validos = df_subgrupos[df_subgrupos['cultivo'] == valor_cultivo]['subgrupo'].tolist()
        
        if valor_subgrupo in subgrupos_validos:
            return valor_subgrupo
        return ''
    
    # Aplicar la validación a cada fila del DataFrame
    df['SUB GRUPO'] = df.apply(validar_fila, axis=1)
...
def validar_dosis_productos(df,conn):
...
def validar_momentos(df,conn):
    ...
    for columna in columnas_momento:
        df[columna] = df[columna].apply(lambda x: x if x in momentos else '')
...
# SUBGRUPO CULTIVO
def validar_subgrupo_cultivo(df,conn):
    
    subgrupo_query = """SELECT subgrupo FROM datcrea_tablas.subgrupos_cultivos"""
    subgrupo = pd.read_sql(subgrupo_query,con=conn)
    subgrupo = subgrupo['subgrupo'].tolist()
...
# SUBGRUPO CULTIVOS
def cargar_subgrupos_cultivos(conn):
    """Carga los subgrupos de cultivos desde la base de datos y los organiza en un diccionario."""
    ...
    return subgrupos_cultivos_dict

def validar_subgrupo(row, subgrupos_cultivos_dict):
    """Valida el subgrupo de cultivo para una fila dada."""
    valor_cultivo = row['Cultivo']
...
def validar_subgrupo_cultivo(df, conn):
    """Valida y corrige los valores de SubgrupoCultivo en el DataFrame."""
    # Cargar el diccionario de subgrupos de cultivos
    subgrupos_cultivos_dict = cargar_subgrupos_cultivos(conn)
    
    # Validar cada fila del DataFrame
    df['SubgrupoCultivo'] = df.apply(validar_subgrupo, axis=1, subgrupos_cultivos_dict=subgrupos_cultivos_dict)
    
    return df

# VALIDAR ANTECESORES
def validar_antecesores(df,conn):
    
    antecesores_query = """SELECT valor FROM datcrea_tablas.variables_generales WHERE variable = 'antecesor'"""
    antecesor = pd.read_sql(antecesores_query,con=conn)
    antecesor = antecesor['valor'].tolist()
    df['CultivoAntecesor'] = ['' if x not in antecesor else x for x in df['CultivoAntecesor']]
    return df
...
df['Version Biotecnológica'] = df.apply(validar_biotecnologia, axis=1)
